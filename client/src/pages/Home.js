...
const [pnfVisible,setPnfVisible] = useState(true)

//get weather details function
  const getWeatherDetails = async (e) =>{
    e.preventDefault()
    try {
        const res = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${process.env.REACT_APP_WEATHER_API_KEY}`);
...
<div className='text-center'>
    ...
    {/* converting default temperature kelvin into celsius */}
    <h2>{main}</h2>
